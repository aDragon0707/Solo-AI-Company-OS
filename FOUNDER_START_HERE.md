# Founder Start Here

**Purpose:** A short orientation for using Solo-AI-Company-OS.

Read this before opening AI windows or assigning work.

---

## 1. One-Sentence Version

Solo-AI-Company-OS helps a solo founder operate a company with AI employees while preserving decision authority, project truth, and accountability.

---

## 2. The First Files To Read

| Question | File |
|---|---|
| What have I formally decided? | `01_Founder/FOUNDER_Decision_Log.md` |
| What is the current operating state? | `02_Dashboards/DASHBOARD_Company_State.md` |
| How is the vault linked in Obsidian? | `02_Dashboards/DASHBOARD_Link_Map.md` |
| How may AI maintain the text system? | `00_System_Brain/AI_Text_Maintenance_Protocol.md` |
| What should I do in the first session? | `FIRST_30_MINUTES.md` |
| What should I open next? | `OBSIDIAN_HOME.md` |

If you are tired, busy, or context-switching, start with these files.

---

## 3. The Three Founder Ledgers

| Ledger | Use |
|---|---|
| `FOUNDER_Thinking_Inbox.md` | Messy thoughts, concerns, ideas, and undecided questions |
| `FOUNDER_Decision_Log.md` | Confirmed decisions that AI employees must obey |
| `FOUNDER_Retrospective_Log.md` | Review of judgment, mistakes, learning, and next actions |

AI worklogs do not replace founder decisions.

---

## 4. The Five AI Employees

| AI | Use When You Need |
|---|---|
| AI-01 | priorities, routing, coordination, state checks |
| AI-02 | project truth, implementation, verification |
| AI-03 | customer development, outreach, offers |
| AI-04 | risk review, claim boundaries, sensitive wording |
| AI-05 | learning, architecture explanation, homework review |

Open the right AI for the job. Do not ask every AI to do everything.

---

## 5. What AI Must Not Do

AI must not:

- turn drafts into founder decisions
- make customer promises without approval
- hide uncertainty
- invent missing source truth
- use market pressure to override verification
- continue another AI's work without a clear handoff

---

## 6. First Setup Checklist

1. Open `FIRST_30_MINUTES.md`.
2. Rename placeholders: `[Company Name]`, `[Project Name]`, `[Product Name]`.
3. Fill in your first founder decisions.
4. Update `02_Dashboards/DASHBOARD_Company_State.md` with the current project state.
5. Open `02_Dashboards/DASHBOARD_Link_Map.md` in Obsidian and confirm the graph has a usable starting shape.
6. Customize AI employee role prompts.
7. Create the first worklog entry.
8. Build one account workspace from `03_Company/Accounts/MOC_Account_TEMPLATE.md`.
9. Use AI-05 to map your project before asking for code-level explanations.
10. If your vault grows, ask an AI assistant to follow `00_System_Brain/AI_Text_Maintenance_Protocol.md`.

---

## 7. Founder Command To AI-01

Copy this:

```text
AI-01, read the founder decision log, Obsidian home, company maps, and worklog index. Tell me the top 3 priorities, current blockers, required founder decisions, and which AI owns the next step.
```

For Obsidian link cleanup, copy the prompt in:

```text
00_System_Brain/AI_Obsidian_Link_Maintenance_Prompt.md
```

For broader dashboard, MOC, worklog-index, and handoff cleanup, use:

```text
00_System_Brain/AI_Text_Maintenance_Protocol.md
```
